...
.compute_IDset_DeltaRM_fixedS <- function(s, Mbar, max_positive,
  ...
  A_RM_s = rbind(A_RM_s, prePeriodEqualityMat)
  d_RM = c(d_RM, trueBeta[1:numPrePeriods])
  dir_RM = c(dir_RM, rep("==", NROW(prePeriodEqualityMat)))
...
.computeConditionalCS_DeltaRM_fixedS <- function(s, max_positive, Mbar,
  ...
  if (postPeriodMomentsOnly & numPostPeriods > 1){
    postPeriodIndices <- (numPrePeriods +1):NCOL(A_RM_s)
    postPeriodRows <- which( rowSums( A_RM_s[ , postPeriodIndices] != 0 ) > 0 )
    rowsForARP <- postPeriodRows
  } else{
    rowsForARP <- 1:NROW(A_RM_s)
  }
  ...
  if (numPostPeriods == 1) {
    if (hybrid_flag == "LF") {
      ...
      lf_cv = .compute_least_favorable_cv(X_T = NULL, sigma = A_RM_s %*% sigma %*% t(A_RM_s), hybrid_kappa = hybrid_kappa)
...
computeConditionalCS_DeltaRM <- function(betahat, sigma, numPrePeriods, numPostPeriods,
  ...
  CIs_RM_plus_allS = matrix(0, nrow = gridPoints, ncol = length(s_indices))
  CIs_RM_minus_allS = matrix(0, nrow = gridPoints, ncol = length(s_indices))
  ...
  CIs_RM_plus_maxS = apply(CIs_RM_plus_allS, MARGIN = 1, FUN = max)
  ...
  if (returnLength) {
    gridLength <- 0.5 * ( c(0, diff(CI_RM$grid)) + c(diff(CI_RM$grid), 0 ) )
    return(sum(CI_RM$accept*gridLength))
  } else {
    return(CI_RM)
  }
}
