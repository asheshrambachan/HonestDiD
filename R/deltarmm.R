...
.compute_IDset_DeltaRMM_fixedS <- function(s, Mbar, max_positive,
  ...
  A_RMM_s = rbind(A_RMM_s, prePeriodEqualityMat)
  d_RMM = c(d_RMM, trueBeta[1:numPrePeriods])
  dir_RMM = c(dir_RMM, rep("==", NROW(prePeriodEqualityMat)))
...
.computeConditionalCS_DeltaRMM_fixedS <- function(s, max_positive, Mbar,
  ...
  if (postPeriodMomentsOnly & numPostPeriods > 1){
    postPeriodIndices <- (numPrePeriods +1):NCOL(A_RMM_s)
    postPeriodRows <- which( rowSums( A_RMM_s[ , postPeriodIndices] != 0 ) > 0 )
    rowsForARP <- postPeriodRows
  } else{
    rowsForARP <- 1:NROW(A_RMM_s)
  }
  ...
  if (numPostPeriods == 1) {
    if (hybrid_flag == "LF") {
      ...
      lf_cv = .compute_least_favorable_cv(X_T = NULL, sigma = A_RMM_s %*% sigma %*% t(A_RMM_s), hybrid_kappa = hybrid_kappa)
...
computeConditionalCS_DeltaRMM <- function(betahat, sigma, numPrePeriods, numPostPeriods,
  ...
  CIs_RMM_plus_allS = matrix(0, nrow = gridPoints, ncol = length(s_indices))
  CIs_RMM_minus_allS = matrix(0, nrow = gridPoints, ncol = length(s_indices))
  ...
  CIs_RMM_plus_maxS = apply(CIs_RMM_plus_allS, MARGIN = 1, FUN = max)
  ...
  if (returnLength) {
    gridLength <- 0.5 * ( c(0, diff(CI_RMM$grid)) + c(diff(CI_RMM$grid), 0 ) )
    return(sum(CI_RMM$accept*gridLength))
  } else {
    return(CI_RMM)
  }
}
